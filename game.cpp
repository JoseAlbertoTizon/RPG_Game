...
//    skeletons[1].sprite.setPosition(400, 200);
    ...
    while(window.isOpen()) {
        ...
        if (event.type == sf::Event::MouseButtonPressed and not character.is_moving) {
            ...
            if(event.mouseButton.button == sf::Mouse::Right and released_right) {
                sf::Vector2i mousePosition = sf::Mouse::getPosition(window);
                for (int k = 0; k < graph.vertices.size(); ++ k) {
                    if (graph.vertices[k].getGlobalBounds().contains(mousePosition.x, mousePosition.y)) {
                        std::cout << graph.edges[character.from_circle][k];
